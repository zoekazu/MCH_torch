        ...
        with torch.no_grad():
            ...
            for test_loader, test_dataset_name in zip(test_loaders, TEST_DATASET_NAMES):
                ...
                for (cnn_input, cnn_label), test_file_name in zip(test_loader, test_file_names):
                    ...
                    outim_name = './{0}/{1}/{2}/{3}_{1}.bmp'.format(RESULT_DIR, epoch, test_dataset_name, image_name)
                    cv2.imwrite(outim_name, cnn_output)

                    df_epoch = df_epoch.append(
                        pd.Series([image_name, psnr_result, ssim_result], index=df_epoch.columns), ignore_index=True)
                ...
                df_epoch = df_epoch.append(
                    pd.Series(['average', psnr_average, ssim_average], index=df_epoch.columns), ignore_index=True)

                # save df_epoch
                df_epoch.to_pickle('{0}/{1}/{2}/df_epoch_{1}.pkl'.format(RESULT_DIR, epoch, test_dataset_name))
                df_epoch.to_csv('{0}/{1}/{2}/df_epoch_{1}.csv'.format(RESULT_DIR, epoch, test_dataset_name))
